...
router.post('/login', async (req, res) => {
    try {
        ...
        const token = jwt.sign({ id: user.id, email: user.email }, process.env.JWT_SECRET || 'tu_clave_secreta_muy_segura', { expiresIn: '24h' });
        res.json({
...
router.post('/admin/login', async (req, res) => {
    try {
        ...
        if (email === 'admin@admin.com' && password === 'admin123') {
            const token = jwt.sign({ id: 'admin', email, role: 'admin' }, process.env.JWT_SECRET || 'tu_clave_secreta_muy_segura', { expiresIn: '24h' });
            return res.json({
                message: 'Login exitoso',
                token,
